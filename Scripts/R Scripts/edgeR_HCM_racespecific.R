...
# Filter pdata to include only patients with HCM
pdata_hcm <- pdata[pdata$etiology == "HCM", ]

# Filter pdata_hcm to include only African American and Caucasian patients
pdata_filtered <- pdata_hcm[pdata_hcm$race %in% c("AA", "Caucasian"), ]
...
significant_genes <- DE_genes[DE_genes$PValue < 0.05, ]
...
output_file <- paste0(output_folder, "SignficantGenes_HCM_AAvsCaucasian")
...
common_gene_names <- read.xlsx("/Users/Rachel/Downloads/CLASSES/20.440/project/440project/R code outputs/signficantgenes_HCM_AAvsCaucasian.xlsx")
...
volcano_plot <- function(DE_genes, threshold = 0.05, logFC_threshold = 0.3) {
  ...
  plot(DE_genes$logFC, -log10(DE_genes$AdjustedPValue), pch = 20, col = ifelse(DE_genes$AdjustedPValue > threshold, "grey",
       ...
       main = "DEGs in AA HCM Patients Relative to Caucasian HCM Patients")
