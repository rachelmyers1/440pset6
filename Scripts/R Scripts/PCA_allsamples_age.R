...
data_for_pca <- combined_data_with_annotation[, -c(1:4)] # Exclude first four columns (Sample_title, Sex, Race, Etiology)
...
na_or_non_numeric <- pdata$age[!is.na(pdata$age) & !is.numeric(pdata$age)]
...
age_intervals <- cut(pdata$age, breaks = c(0, 35, 56, 80, Inf), labels = c("0-35", "35-56", "57-80", "81+"))
...
ggplot(pc_scores, aes(x = PC1, y = PC2, color = Age)) +
  geom_point() +
  labs(title = "PCA of Patients by Age Group",
       x = "PC1",
       y = "PC2") +
  scale_color_manual(values = c("#45a989", "purple", "blue", "#bf4f16")) + # Define colors for age groups
  theme_minimal() +
...
# Create eight age intervals
age_intervals <- cut(pdata$age, breaks = c(0, 16, 30, 55, 60, 65, 70, 75, Inf), labels = c("0-16", "17-30", "31-55", "56-60", "61-65", "66-70", "71-75", "75+"))
...
ggplot(pc_scores, aes(x = PC1, y = PC2, color = age)) +
  ...
  labs(title = "PCA of Patients by Age Group",
       x = "PC1",
       y = "PC2") +
  scale_color_manual(values = c("#45a989", "purple", "blue", "#d03161","#ee8080","#bfd8d1","#178a94","#2b374b")) + # Define colors for age groups
  theme_minimal() +
  theme(legend.position = "top")
...
age_intervals <- cut(pdata$age, breaks = c(0, 35, Inf), labels = c("0-35", "36+"))
...
ggplot(pc_scores, aes(x = PC1, y = PC2, color = age)) +
  ...
  labs(title = "PCA of Patients by Age Group",
       x = "PC1",
       y = "PC2") +
  scale_color_manual(values = c("#45a989", "purple")) + # Define colors for age groups
  theme_minimal() +
  theme(legend.position = "top")



# Do four other age intervals
age_intervals <- cut(pdata$age, breaks = c(0, 35, 50, 77, Inf), labels = c("0-35", "36-50","51-77", "78+"))
...
ggplot(pc_scores, aes(x = PC1, y = PC2, color = age)) +
  ...
  labs(title = "PCA of Patients by Age Group",
       x = "PC1",
       y = "PC2") +
  scale_color_manual(values = c("#45a989", "purple", "blue", "orange")) + # Define colors for age groups
  theme_minimal() +
  theme(legend.position = "top")
...
ggplot(pdata, aes(x = age)) +
  geom_histogram(bins = 2, fill = "blue", color = "black") +
