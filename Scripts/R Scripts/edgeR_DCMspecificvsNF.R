...
pdata_filtered <- pdata[pdata$etiology %in% c("DCM", "NF"), ]
...
design <- model.matrix(~ etiology, data = pdata_filtered)
...
output_file <- paste0(output_folder, "SignficantGenes_DCMspecific_DCMvsNF")
...
volcano_plot <- function(DE_genes, threshold = 0.05, logFC_threshold = 0.3) {
  ...
  plot(DE_genes$logFC, -log10(DE_genes$AdjustedPValue), pch = 20, col = ifelse(DE_genes$AdjustedPValue > threshold, "grey",
       ...
       main = "DCM-Specific DEGs in DCM vs NF Patients")
