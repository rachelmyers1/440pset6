...
# Load the CSV file
combined_data_with_annotation <- read.csv("/Users/Rachel/Downloads/CLASSES/20.440/project/440project/R code outputs/combined_data_with_annotation.csv")

# Extract numerical data for PCA
data_for_pca <- combined_data_with_annotation[, -c(1:4)] # Exclude first three columns (Sample_title, Sex, Race)
...
# Define custom colors for etiologies
#etiologies_colors <- c("#797ef5", "#22bcbc", "#17a7ec", "#1e2f97", "#b512cf")
etiologies_colors <- c('red', 'red', 'blue', 'red', 'red')


# Plot PCA results with custom etiology colors
ggplot(pc_scores, aes(x = PC1, y = PC2, color = Etiology)) +
  geom_point() +
  labs(title = "PCA of Samples by Etiology",
       x = "PC1",
       y = "PC2") +
  scale_color_manual(values = etiologies_colors) + # Set custom colors for etiologies
  theme_minimal() +
