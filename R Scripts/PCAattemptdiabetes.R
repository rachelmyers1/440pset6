...
data_for_pca <- combined_data[, -1] # Exclude first four columns (Sample_title, Sex, Race, Etiology)
...
pc_scores$Diabetes <- pdata$Diabetes

# Plot PCA results
ggplot(pc_scores, aes(x = PC1, y = PC2, color = Diabetes)) +
  geom_point() +
  labs(title = "PCA of Samples by Diabetes Status",
       x = "PC1",
       y = "PC2") +
