# Set file paths
hcm_hits_file <- "/Users/Rachel/Downloads/CLASSES/20.440/project/440project/R code outputs/SignficantGenes_HCMvsNF"
dcm_hits_file <- "/Users/Rachel/Downloads/CLASSES/20.440/project/440project/R code outputs/SignficantGenes_DCMvsNF"

# Read the gene lists
hcm_genes <- read.csv(hcm_hits_file, header = TRUE, stringsAsFactors = FALSE)
dcm_genes <- read.csv(dcm_hits_file, header = TRUE, stringsAsFactors = FALSE)

# Find genes unique to DCM
unique_dcm_genes <- setdiff(dcm_genes, hcm_genes)

# Write the unique DCM genes to a new CSV file
write.csv(data.frame(GeneID = unique_dcm_genes), file = "Unique_DCM_Genes.csv", row.names = FALSE)
...
# Load counts file
counts <- read.csv("/Users/Rachel/Downloads/CLASSES/20.440/project/440project/R code/baySeq/combined_data.csv")
...
# Subset counts to only include genes that are unique to dcm
counts_filtered <- counts[counts$ENSGene %in% unique_dcm_genes$x, ]

# Subset counts to include only samples corresponding to pdata_filtered
counts_filtered <- counts_filtered[, colnames(counts) %in% pdata_filtered$sample_name]
...
DE_genes$AdjustedPValue <- p.adjust(DE_genes$PValue, method = "BH")
...
output_file <- paste0(output_folder, "SignficantGenes_DCMspecific_AAvsCaucasian")
...
common_gene_names <- read.xlsx("/Users/Rachel/Downloads/CLASSES/20.440/project/440project/R code outputs/signficantgenes_DCMspecific_AAvsCaucasian.xlsx")
...
volcano_plot <- function(DE_genes, threshold = 0.05, logFC_threshold = 0.3) {
  ...
  plot(DE_genes$logFC, -log10(DE_genes$AdjustedPValue), pch = 20, col = ifelse(DE_genes$AdjustedPValue > threshold, "grey",
       ...
       main = "DCM-Specific DEGs in AA Patients Relative to Caucasian Patients")
