...
data_for_pca <- combined_data_with_annotation[, -c(1:3)] # Exclude first three columns (Sample_title, Sex, Race)
...
# Add Race information to PC scores dataframe
pc_scores$Race <- combined_data_with_annotation$Race

# Plot PCA results
ggplot(pc_scores, aes(x = PC1, y = PC2, color = Race)) +
  geom_point() +
  labs(title = "PCA Plot of Samples by Race",
       x = "PC1",
       y = "PC2") +
