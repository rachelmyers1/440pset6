...
data_for_pca <- filtered_data[, -c(1:4)] # Exclude first three columns (Sample_title, Sex, Race)
...
pc_scores$Race <- filtered_data$Race

# Plot PCA results
ggplot(pc_scores, aes(x = PC1, y = PC2, color = Race)) +
  geom_point() +
  labs(title = "PCA of DCM Patients by Race",
       x = "PC1",
       y = "PC2") +
  theme_minimal() +
